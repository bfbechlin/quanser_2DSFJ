...
/**
	...
	DTR:: 8, 16, 24, 32 bit configurable. To get values from MOSI bus. useful
		to transfer value to CNTR register.
	CNTR:: 8, 16, 24, 32 bit configurable. Counter register that transfer the
		data to OTR register and in this register it's write on MISO bus.
	STR:: 8-bit. Status register.
	MDRO::	8-bit. Mode register 0.
	MDR1:: 8-bit. Mode register 0.
...
/**
	Inicializes the SPI communication parameters that LS7366R supports.
**/
int LS7366R_init(int spi_fd);

/**
	No operand register instructions.
**/
int LS7366R_clear(int spi_fd, int cs_fd, unsigned char op_code);

/**
	Write a byte in a write call instructuin for a register.
**/
int LS7366R_write(int spi_fd, int cs_fd, unsigned char op_code, unsigned char data);

/**
	Read 'bytes' in a read call instruction for a register.
**/
